...
export class SecureHttpTool {
  private readonly logger = new Logger(SecureHttpTool.name);
  ...
  @Tool({
    ...
    parameters: z.object({
      url: z
        ...
        .url()
        .describe(
          "The full URL to call (e.g. https://api.example.com/users/123)",
        ),
      method: z
      ...
      headers: z
        ...
        .optional()
        .describe(
          'HTTP headers to include (e.g. {"Authorization": "Bearer token", "Content-Type": "application/json"})',
        ),
      body: z
        .any()
        .optional()
  ...
  async executeHttp(
    ...
    const middlewareResult =
      await this.middlewareService.processRequest(config);

    if (!middlewareResult.allowed) {
      if (middlewareResult.middleware) {
        this.logger.warn(
          `Request denied by middleware "${middlewareResult.middleware.title}": ${method} ${url}`,
        );
      } else {
        this.logger.warn(
          `Request denied (no matching middleware): ${method} ${url}`,
        );
      }
    ...
    this.logger.log(
      `Request allowed by middleware "${middlewareResult.middleware.title}"`,
    );
    ...
    try {
      ...
      return {
        ...
        allowed: true, // Middleware allowed it, but network request failed
        matchedMiddleware: middlewareResult.middleware
          ? {
              title: middlewareResult.middleware.title,
              description: middlewareResult.middleware.description,
  ...
  @Tool({
    ...
    description:
      "List all configured middlewares with their URL patterns and descriptions.",
    parameters: z.object({}),
  })
  async listMiddlewares() {
    ...
    if (middlewares.length === 0) {
      return {
        message:
          "No middlewares configured. Create .js files in the middlewares directory.",
        middlewares: [],
      };
    }
  ...
  @Tool({
    ...
    description:
      "Reload all middleware files from the middlewares directory. Use this after adding or modifying middleware files.",
    parameters: z.object({}),
  })
